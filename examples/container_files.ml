...
let () =
  ...
  let event_codec =
    Avro.Codec.record (Avro.Type_name.simple "Event")
      (fun timestamp user_id event_type -> ({ timestamp; user_id; event_type } : event))
    |> Avro.Codec.field "timestamp" Avro.Codec.long (fun (e : event) -> e.timestamp)
    |> Avro.Codec.field "user_id" Avro.Codec.int (fun (e : event) -> e.user_id)
    |> Avro.Codec.field "event_type" Avro.Codec.string (fun (e : event) -> e.event_type)
  ...
  List.iter (fun event -> Avro.Container_writer.write writer event) events;
  ...
  let reader = Avro.Container_reader.open_file
    ~path:filename_null
    ~codec:event_codec
    () in

  Printf.printf "   Codec: %s\n" (Avro.Container_reader.codec_name reader);
  Printf.printf "   Events:\n";

  let count = ref 0 in
  Avro.Container_reader.iter (fun event ->
    incr count;
  ...
  let writer_deflate = Avro.Container_writer.create
  ...
  List.iter (fun event -> Avro.Container_writer.write writer_deflate event) large_events;
  Avro.Container_writer.close writer_deflate;
  ...
  let reader_deflate = Avro.Container_reader.open_file
    ~path:filename_deflate
    ~codec:event_codec
    () in

  (* Count events by type *)
  let event_counts = Avro.Container_reader.fold (fun acc event ->
  ...
  Avro.Container_reader.close reader_deflate;
  ...
  let writer_blocks = Avro.Container_writer.create
  ...
  Avro.Container_writer.write_block writer_blocks batch1;

  let batch2 = Array.init 15 (fun i ->
    { timestamp = Int64.of_int (i + 100); user_id = i + 100; event_type = "batch2" }
  ) in
  Avro.Container_writer.write_block writer_blocks batch2;

  Avro.Container_writer.close writer_blocks;
  ...
  let reader_blocks = Avro.Container_reader.open_file
    ~path:filename_blocks
    ~codec:event_codec
    () in

  let block_num = ref 0 in
  Avro.Container_reader.iter_blocks (fun block ->
    incr block_num;
    Printf.printf "   Block %d: %d events\n" !block_num (Array.length block);
  ) reader_blocks;

  Avro.Container_reader.close reader_blocks;

  (* Example 7: Using sequences for lazy processing *)
  Printf.printf "\n7. Lazy sequence processing:\n";

  let reader_seq = Avro.Container_reader.open_file
    ~path:filename_deflate
    ~codec:event_codec
    () in

  (* Process only first 5 events using sequence *)
  let first_5 = Avro.Container_reader.to_seq reader_seq
    |> Seq.take 5
    |> List.of_seq in
  ...
  Avro.Container_reader.close reader_seq;

  (* Example 8: Check available compression codecs *)
  Printf.printf "\n8. Available compression codecs:\n";

  let codecs = Avro.Codec_registry.list () in
  List.iter (fun codec ->
    Printf.printf "   - %s\n" codec
  ) codecs;
  ...
  let writer_metadata = Avro.Container_writer.create
  ...
  Avro.Container_writer.write writer_metadata { timestamp = 0L; user_id = 0; event_type = "test" };
  Avro.Container_writer.close writer_metadata;

  (* Read metadata back *)
  let reader_metadata = Avro.Container_reader.open_file
    ~path:filename_metadata
    ~codec:event_codec
    () in

  let metadata = Avro.Container_reader.metadata reader_metadata in
  ...
  Avro.Container_reader.close reader_metadata;
